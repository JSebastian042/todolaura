...
export default function RegaloPage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-950 via-rose-950 to-pink-950 flex flex-col items-center justify-center p-4 sm:p-8 select-none relative overflow-hidden">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-pink-500/15 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-10 right-10 w-80 h-80 bg-amber-500/10 rounded-full blur-3xl pointer-events-none" />

      {/* Contenedor Principal */}
      <div className="relative bg-rose-950/40 backdrop-blur-xl p-8 sm:p-12 rounded-[2.5rem] shadow-[0_20px_60px_rgba(244,114,182,0.15)] max-w-lg w-full text-center border border-pink-500/20 flex flex-col items-center transition-all duration-500">
        ...
        <p className="text-pink-200/80 text-sm sm:text-base mb-8 max-w-xs font-medium">
          {abierto 
        ...
        <div className="relative w-64 h-64 sm:w-72 sm:h-72 my-4 flex items-center justify-center">
          ...
          <div className={`transition-all duration-700 transform ${abierto ? 'scale-100 opacity-100 translate-y-0' : 'scale-75 opacity-40 translate-y-4 filter blur-sm'}`}>
            <svg viewBox="0 0 200 240" className="w-56 h-56 sm:w-64 sm:h-64 drop-shadow-[0_10px_20px_rgba(244,114,182,0.3)]">
              {/* Florero / Lazo */}
              ...
              {/* TULIPÁN CENTRO (Amarillo Cálido) */}
        ...
        <button
          ...
          className="my-6 px-8 py-3.5 bg-gradient-to-r from-pink-500 to-rose-400 hover:from-pink-600 hover:to-rose-500 text-white font-bold rounded-full text-base transition-all transform hover:scale-105 active:scale-95 shadow-[0_10px_25px_rgba(244,114,182,0.4)] flex items-center gap-2"
        >
        ...
        <Link
          href="/dashboard"
          className="mt-2 px-8 py-3.5 bg-gradient-to-r from-pink-500/20 to-rose-500/20 hover:from-pink-500/30 hover:to-rose-500/30 text-pink-200 font-bold rounded-full text-base border border-pink-400/30 transition shadow-lg w-full backdrop-blur-md"
        >
